...
public class PathSetter {
	...
	private final static Logger logger = Logger.getLogger(PathSetter.class);
					...
					try {
						...
						if (subObj == null) {
							...
							if (type.isArray()) {
								...
								if (++i < bef)
									source = getOrCreateListItem((List) subObj, ReflectUtil.getReturnCollectionType(getMethod), Integer
											.valueOf(keys[i]));
		...
		if (item == null) {
			try {
				...
				if (logger.isDebugEnabled())
					logger.error("can not create instance of " + type);
				return null;
			}
		}
		...
		if (obj != null) {
			...
			try {
				...
				if (value == null) {
					...
					if (setMethod == null) {
						logger.error("无法从类:" + clazz + "的实例中赋予对应属性:" + fieldName + "的值");
						return;
					}
